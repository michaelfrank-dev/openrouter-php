...
/**
 * Class ImageSseStreamParser
 *
 * Parses incoming server-sent event image streaming payloads.
 *
 * @package MichaelFrank\OpenRouter\Http
 */
final class ImageSseStreamParser
{
    /**
     * Parses the streaming SSE response, yielding ImageStreamingResponse objects.
     *
     * @param \Psr\Http\Message\ResponseInterface $response
     * @return \Generator<int, ImageStreamingResponse, mixed, null>
    ...
    public function parse(\Psr\Http\Message\ResponseInterface $response): \Generator
    {
        ...
        if ($isJson) {
            ...
            throw new ApiResponseException(
                'Expected event stream response (text/event-stream), but received application/json. The selected model or provider does not support streaming image generation.',
                $bodyContent,
                $metadata
            );
        }
        ...
        while (!$body->eof()) {
            try {
                ...
                throw new NetworkException(
                    'Network error reading image completion stream: ' . $e->getMessage(),
                    $metadata,
                    $e
                );
            }
            ...
            while (($pos = strpos($buffer, "\n")) !== false) {
                ...
                if (is_array($payload)) {
                    ...
                    if (isset($payload['data']['type']) && $payload['data']['type'] === 'error') {
                        $errorObj = $payload['data']['error'] ?? [];
                        $errorMessage = 'Unknown stream error';
                    ...
                    yield ImageStreamingResponse::fromArray($payload, $metadata);
                }
            }
        }
